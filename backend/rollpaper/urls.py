...
urlpatterns = [
    ...
    path('papers/<str:paper_id>/photos',views.photo),#사진 추가
    path('papers/<str:paper_id>/xyphotos',views.xyphotos), #image 정보들 null값인거 채워넣기
    path('papers/<str:paper_id>/memos',views.memo), #메모 생성
    path('papers/memos/<int:memo_id>',views.memo_delete), #메모 삭제
    path('papers/<str:paper_id>/stickers',views.stickers), #스티커를 추가
    path('users/<int:user_id>', views.my_page, name="my_page"), #user_id라는 것이 존재x, User 테이블의 id를 입력받음
    path('papers/<str:paper_id>/',views.get_paper), #paper의 memo,image,sticker의 정보를 가져옵니다
    path('papers/cartoons',views.cartoon_id),
    path('papers/sticker_list',views.get_stickers), #스티커 가져오기
    path('papers/cartoons/results/<str:task_id>/<str:paper_id>',views.cartoon_result),
